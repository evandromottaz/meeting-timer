import { FormValuesProps, limitTimeType, pdfTextType } from "../types/meetingSchedule";
...
const ChristianLife = () => {
	...
	return (
		<Fieldset title="Nossa Vida Cristã">
			{fields.map((item, i) => {
				const { id, placeholder, pdfText, limitTime } = item;

				return (
					<Fragment key={id}>
						<h3 className="heading border-top pt-3">{pdfText} - ({limitTime}min)</h3>
						...
						<section className="d-flex align-items-center">
							...
							/>
							<Timer name={`christianLife.fields[${i}].time`} />
						</section>
